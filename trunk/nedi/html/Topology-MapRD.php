...
<OPTION VALUE="2" <?=($lev == "1")?"selected":""?>>city
<OPTION VALUE="3" <?=($lev == "2")?"selected":""?>>building
<OPTION VALUE="4" <?=($lev == "3")?"selected":""?>>device
</select>
<INPUT type="checkbox" name="geo" <?=$geo?> title="Geographic Map"> Geo
<INPUT type="checkbox" name="dot" <?=$dot?> title="Geographic Map"> Dot
</td></tr>
<tr><td>Offset</td><td>
...
function Drawitem($x,$y,$opt,$label,$typ) {
	...
	global $dot,$fmt,$dev,$loi,$ipi,$redbuild;
...
# Build Layout
	Drawlevel($locs,(intval($xm/2) + $xo), (intval($ym/2) + $yo),1 );
}

#===================================================================
# Recursevly draw a map level
function Drawlevel($locs,$xs,$ys,$mylev) {

	global $link,$locsep,$resmsg;
	global $lev,$loi,$liwgt,$len;
	global $mapbg,$mapinfo,$mapframes,$maplinks,$mapitems,$imgmap;

	$nextlev = $mylev;
	$nextlev++;
	echo "L:$mylev $nextlev ";
	$nloc   = count($locs);
	if($nloc == 1){
		$scalx = 0;
		$scaly = 0;
	...
	foreach(Arrange($locs) as $loc){
		...
		if( 1 ){
			...
			$x = intval($xs + $len * cos($phi) * $scalx / $locw);
			$y = intval($ys + $len * sin($phi) * $scaly / $locw);
		}
		if($mylev < $lev){
			if ($loi and count($locs[$loc]) > 1){$mapinfo .= Drawitem($x,$y,'0',"$loc",'ci');}
			Drawlevel($locs[$loc],$x,$y,$nextlev);
		}else{
			$mapitems .= Drawitem($x,$y,$locs[$loc],$loc,$lev);
			$area = ($x-20) .",". ($y-20) .",". ($x+20) .",". ($y+20);
			$imgmap .= "<area href=?flt=". rawurlencode("$loc$locsep") ."&lev=$nextlev&loi=1&draw=1 coords=\"$area\" shape=rect title=\"Show\">\n";
		}
	}
}

...
# 30/08/07	implemented CSS scheme (Remo Rickli)
*/
	...
	<tr class="<?=$modgroup[$self]?>1">
		<th width="80">
			<a href="<?=$_SERVER['PHP_SELF'] ?>">
				<img src="img/32/flop.png" title="Create a config file archive or export/backup the NeDi database.">
			</a>
		</th>
...
if($action == "export") {
	...
	elseif(strtoupper(substr($query, 0, 43)) == "SELECT DEVICE, CONFIG, TIME FROM CONFIGS") {
		...
		echo "<div class=\"textpad txta\"><h3>Log</h3>\n";
		...
		echo "<p><a href=\"".$archive."\">Download your NeDi device config archive</a></div>\n";
	...
	// For any other SQL query this is processed
		...
		// This is the end of the output table. It also contains the link to the archive
		echo "<p><a href=\"".$archive."\">Download your NeDi CSV export</a></div>\n";
...
// If the "SQL Dump" radio button has been selected
	...
	// This is the end of the output table. It also contains the link to the archive
	echo "<p><a href=\"".$archive."\">Download your NeDi database dump</a></div>\n";
...
echo "<h2>NeDi Database Layout</h2>\n";
	...
	while($tab = DbFetchRow($res)){
		...
		echo "<th colspan=3>$tab[0]</th><th>NULL</th><th>KEY</th></tr>\n";
		$cres = DbQuery(GenQuery($tab[0], "c"), $dblink);
		$row = 0;
		while($c = DbFetchRow($cres)){
			if ($row % 2){$bg = "txta"; $bi = "imga";}else{$bg = "txtb"; $bi = "imgb";}
			echo "<tr class=\"$bg\"><th class=\"$bi\">\n";
			echo "$row</th><td class=\"drd\">$c[0]</td><td>$c[1]</td><td class=\"prp\">$c[2]</td><td class=\"blu\">$c[3]</td></tr>\n";
			$row++;
		}
...
<table class="content" >
<tr class="<?=$modgroup[$self]?>2"><td><?=DbNumRows($nres)?> Records</td></tr>
</table>

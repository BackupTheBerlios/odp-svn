...
if ($opt{t}) {												# Creates incident and bails
	$now = time;
	if( ! &db::Insert('messages','level,time,source,info',"\"10\",\"$now\",\"Test\",\"moni.pl test message\"") ){
		die "DB error messages!\n";
	}
	if( ! &db::Insert('incidents','level,device,deps,firstseen,lastseen,who,time,category,comment',"\"10\",\"Test\",\"0\",\"$now\",\"0\",\"\",\"0\",\"1\",\"\"") ){
		die "DB error incidents!\n";
	}
	&mon::SendSMS("$now: moni.pl test SMS");
	&mon::SendMail("Moni Test","moni.pl test mail");
	die();

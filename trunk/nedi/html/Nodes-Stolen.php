...
<img src="img/32/fiqu.png" title="The upper line show current DB info, the lower reflects the state, when it was reported.">
</a></th>
<th>
...
<?
	...
	while( ($s = @DbFetchRow($res)) ){
		...
		if ($n[5] > $s[6]){$simg = "<img src=\"img/16/lokc.png\" title=\"Reappeared!\">";}

		echo "<tr class=\"$bg\">";
		echo "<th class=\"$bi\" width=120 rowspan=2><a href=Nodes-Status.php?mac=$n[2]><img src=\"img/oui/$img\" title=\"$n[3]\" vspace=8></a><br>$s[2]\n";
		echo "<td>$dbna</td><td>$dbip</td><td>$n[6]</td><td>$n[7]</td><td bgcolor=#$lc>$ls</td>\n";
		echo "<th>$simg <a href=$_SERVER[PHP_SELF]?del=$s[2]><img src=\"img/16/bcnl.png\" onclick=\"return confirm('Delete node $s[2]?')\"></a></th>\n";
...
<table class="content">
<tr class="<?=$modgroup[$self]?>2"><td><?=$row?> Stolen nodes (<?=$query?>)</td></tr>
</table>
	<?
